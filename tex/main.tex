...
\def\FebruarHolidays{}
\def\MärzHolidays{}
\def\AprilHolidays{3,6,31}
\def\MaiHolidays{1,14,25}
\def\JuniHolidays{4}
\def\JuliHolidays{}
\def\AugustHolidays{15}
\def\SeptemberHolidays{33}
\def\OktoberHolidays{3,32}
\def\NovemberHolidays{1}
\def\DezemberHolidays{25,26,32}
...
% holds the result (as digits)
\def\DayNumberOfLastMondayOfDecemberOfLastYear{0}

% sets \DayNumberOfLastMondayOfDecemberOfLastYear for a given year #1
\def\SetDayNumberOfLastMondayOfDecemberOfLastYear#1{%
  \begingroup
  % y = previous year
  \count0=\numexpr#1-1\relax

  % Compute weekday of Dec 31 of year y using:
  % w = ( y + floor(y/4) - floor(y/100) + floor(y/400) + 35 ) mod 7
  % and then w := w+1 so that Sunday=1, Monday=2, ..., Saturday=7
  \count2=\numexpr\count0 + \count0/4 - \count0/100 + \count0/400 + 35\relax
  \count4=\numexpr\count2 - 7*(\count2/7)\relax % 0..6
  \count6=\numexpr\count4 + 1\relax            % 1..7 (Sun=1)

  % offset from Monday (Monday=2 in this scheme)
  \count8=\numexpr\count6-2\relax
  \count10=\numexpr\count8 - 7*(\count8/7)\relax % may be negative
  \ifnum\count10<0 \count10=\numexpr\count10+7\relax\fi

  % last Monday day-of-month
  \count12=\numexpr31-\count10\relax

  % store globally
  \xdef\DayNumberOfLastMondayOfDecemberOfLastYear{\the\count12}%
  \endgroup
}
