...
\newcommand{\YearFontSize}{\fontsize{40pt}{12pt}\selectfont}
...
\newcommand\IfGraphicExistsTF[3]{%
  \begingroup
  \let\Ginput@path\Ginput@path % keep graphicspath
  \edef\Gin@base{#1}%
  % Try to resolve the file the same way \includegraphics does:
  \@ifundefined{Gin@setfile}{%
    % graphicx not ready (shouldn't happen if graphicx loaded)
    \endgroup #3%
  }{%
    \let\Gin@ext\relax
    \let\Gin@sep\relax
    \let\Gin@rule\relax
    \let\Gin@req@bb\relax
    % This sets \Gin@setfile@found and \Gin@setfile@name internally
    \Gin@setfile\Gin@base
    \ifx\Gin@setfile@name\@empty
      \endgroup #3%
    \else
      \endgroup #2%
    \fi
...
\newcommand{\drawMonthName}[2]{
    ...
    \node at
    (\GridWidth / 2 + 3 * \GridStep, \WeekdayNamesHeight + \MonthNameVerticalSpacing)
    { \YearFontSize \textcolor{InactiveDayNumberColor}{\year} };
}
...
\newcommand{\addPicturePage}[2]{%
  ...
  \thispagestyle{empty}%
  \begin{tikzpicture}[remember picture,overlay]
    \IfFileExists{\ImageDir#2}{%
      \node[anchor=north west,inner sep=0] at (current page.north west) {%
        \rotatebox{180}{\includegraphics[width=\paperwidth,height=\paperwidth]{\ImageDir#2}}%
      };%
    }{}%
